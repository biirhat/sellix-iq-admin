...
import { useDeleteEmployee, useEmployees } from "@/helpers/hooks/use-employee";
...
export default function Employees() {
  ...
  const handleDelete = () => {
    ...
    setTimeout(() => {
      deleteEmployeeMutation(target.id, {
        onSuccess: () => {
          showSuccess("Employee deleted", `${target.username} has been removed.`);
        },
        onError: (error) => {
  ...
  const stats = useMemo(() => {
    ...
    const companyAdmins =
      employees?.filter((u) => u.role === "company_admin").length ?? 0;
    const representatives =
      employees?.filter((u) => u.role === "representative").length ?? 0;
  ...
  const filteredUsers = useMemo(() => {
    ...
    return employees.filter((user: any) => {
          ...
          <Popover.Content presentation="bottom-sheet">
            <View className="gap-2 p-2">
              ...
              <Popover.Close asChild>
                <Pressable
                  onPress={() => router.push(`/(app)/employees/user-edit/${user.id}`)}
  ...
  return (
    <>
        ...
        <View className="flex-1 relative">
          ...
          <View style={{ position: "absolute", right: 18, bottom: 30 }}>
            <Button
              size="lg"
              isIconOnly
              onPress={() => router.push("/(app)/employees/addEmployees")}
              style={{
                shadowColor: "#4F46E5",
      ...
      <DeleteConfirmationDialog
        ...
        description="Are you sure you want to delete this employee? This will also remove their access."
